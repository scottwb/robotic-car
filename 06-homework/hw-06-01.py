...
def doit(initial_pos, move1, move2):
    ...
    # Put the moves into an array so we can generically handle N moves
    moves = [float(move1), float(move2)]

    # Create zeroed omega and xi matrices - size based on number of positions
    n_positions = len(moves) + 1
    omega = matrix()
    omega.zero(n_positions, n_positions)
    xi = matrix()
    xi.zero(n_positions, 1)
    ...
    for move in moves:
        ...
        omega.value[i][i+1]   -= 1.0
        omega.value[i+1][i]   -= 1.0
        xi.value[i][0]   -= move
        xi.value[i+1][0] += move
        i += 1
...
# Just hack in real quick the omega and xi I can easily compute
# by hand, just so I can use the matrix library to compute mu to
# get the landmark position. Obviously this was trivial math
# given the example numbers, but working out the matrix was
# more educational.
...
xi = matrix([[-6], [-3], [-2], [4], [12]])
xi.show('Xi:    ')

(omega.inverse() * xi).show('mu:    ')
